select 1
USE "c:\ai\energia4\dia-re.dbf" EXCLUSIVE
select 2
USE "c:\ai\energia4\ranges.dbf" EXCLUSIVE
select 1
go top
maior=0
menor=10000000
...
dimension maiorx(40,6)
dimension menorx(40,6)
dimension quantx(40)
dimension valor(40)
...
? trec
wait
dimension var(trec,7)
go top
y=0
do while .not. eof()
 y=y+1
 store precipitac to var(y,1)
 store ttmax      to var(y,2)
 store ttmin      to var(y,3)
 store sol        to var(y,4)
 store ttmed      to var(y,5)
 store ttumid     to var(y,6)
 store pld        to var(y,7)
 skip
enddo
go top
z=0
...
select 3
USE "c:\ai\energia4\dia-re-02.dbf" EXCLUSIVE && arquivo cujo preço sera previsto
go bottom
...
do while .not. eof()
  ...
  store ttmax      to varx(y,2)
  store ttmin      to varx(y,3)
  store sol        to varx(y,4)
  store ttmed      to varx(y,5)
  store ttumid     to varx(y,6)


 ? var(y,1), var(y,2),var(y,3),var(y,4),var(y,5),var(y,6),
 wait
 select 2
 go top
 y1 = 0
 ...
 do while .not. eof()
   ...
   if (temp1+temp2+temp3+temp4+temp5+temp6)=6
   && ? "Esta no range"
   && ? maisquant,quant
   && wait
    if maisquant<quant
    ...
    ? y,y1,temp1,temp2,temp3,temp4,temp5,temp6,
  ...
  select 3
   replace semana with str(rangecerto,4,0)
 skip
enddo
select 3
go top
do while .not. eof()
 store val(semana) to trange
 select 2
